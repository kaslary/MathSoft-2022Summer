...
bool Manderbrot::stop_criterion(){return flag_stop;};
bool Manderbrot::is_disconvergence(){return flag_disconvergence;};
...
int main(int argc, char *argv[]){
	...
	for (int j = 0; j < height; j++){
		for (int i = 0; i < width; i++){
			...
			double y = oy + lpp * j;
			int pos = width * j + i; 
			Manderbrot man(std::complex<double>{0.0, 0.0},N,std::complex<double>{x, y});
